...
export class TaskListComponent implements OnInit, OnDestroy {
  ...
  ipInfov6: any;
  userIpv6: any;
  filter: 'all' | 'upcoming' | 'overdue' = 'all';
  sortOrder: 'dueDate' | 'priority' = 'dueDate';
    ...
    this.IpService.getUserIp().subscribe(
      (data) => {
        ...
        this.IpService.getUserInfo(this.userIpv4).subscribe(
          ...
          (error) => {
            console.error(
              'Error al obtener la información de IP:',
              error.message
            );
          }
        );
      ...
      next: (response) => {
        ...
        const holidays = Array.isArray(response.response.holidays)
          ? response.response.holidays
              ...
              .filter(
                (holiday: any) =>
                  holiday.date.iso >= today &&
                  holiday.date.iso <= thirtyDaysFromNow
              )
              .sort(
                (a: any, b: any) => a.date.iso.getTime() - b.date.iso.getTime()
              )
